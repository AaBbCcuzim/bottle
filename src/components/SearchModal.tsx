    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20">
      ...
      <div className="relative z-10 w-[32rem] max-h-[32rem] flex flex-col rounded-lg border border-border bg-background shadow-lg">
        <div className="flex border-b border-border">
          <button onClick={() => setTab("current")} className={`flex-1 py-2 text-sm font-medium ${tab === "current" ? "bg-muted text-foreground" : "text-muted-foreground"}`}>
            Current File
          </button>
          <button onClick={() => setTab("global")} className={`flex-1 py-2 text-sm font-medium ${tab === "global" ? "bg-muted text-foreground" : "text-muted-foreground"}`}>
            Global Search
          </button>
        </div>
